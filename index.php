    ...
    case "lgn" :
        ...
        if (validator::isEmpty($username) || validator::isEmpty($password)) {
					if( $api != "" ) {
						...
						echo '{"result":0,"error" :"Login Failed - Username / Password Invalid."}';
						die();
					}
					...
					if ($uid > 0) {
						...
						if( $api != "" ) {
							...
							echo '{"result":1,"servers" : ['.$serverStr.'],"name":"'.session('name').'"}';
							die();
						} else {
        ...
        if (validator::isEmpty($email)) $msg = "Email is required";
            ...
            if ($u_obj->Id() > 0) {
                //if ($u_obj->SetRecoverStamp() == true) {
								...
								$db = new db_helper();
								$db->CommandText("UPDATE users SET password_reset_hash='%s' WHERE id = '%s' ");
								$db->Parameters($token);
								$db->Parameters($u_obj->Id());
								$db->Execute();
								//save token to db as password_reset_hash
								
								sendPasswordResetMail($email, $token, $u_obj->Id());
...
?>
            ...
            <button class="btn btn-lg btn-login btn-block" type="button" onclick='window.location="http://www.modlr.co/"'>Modlr.co</button>
